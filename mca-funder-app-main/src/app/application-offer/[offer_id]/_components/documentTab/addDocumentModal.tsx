...
import { getApplicationOfferStipulationList } from "@/lib/api/applicationStipulations";
...
export default function AddDocumentModal({
  ...
  const [documentStipulations, setDocumentStipulations] = useState<
    Map<string, string>
  >(new Map());
  const [applicationStipulations, setApplicationStipulations] = useState<
    ApplicationStipulation[]
  >([]);
  ...
  const [successDocuments, setSuccessDocuments] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    ...
    getApplicationOfferStipulationList(application._id)
      .then(setApplicationStipulations)
      .catch(() => setApplicationStipulations([]));
  ...
  const handleSubmit = async () => {
    ...
    for (const doc of selectedDocs) {
      try {
        ...
        const newDoc = await createApplicationOfferDocument(
          application._id,
          doc._id,
                    ...
                    <div className="items-center justify-left ml-3 flex-1">
                      ...
                      {selectedDocuments.has(doc._id) && (
                        ...
                        >
                          ...
                          >
                            Document Stipulation Type
                          </label>
                          <select
                            value={documentStipulations.get(doc._id) || ""}
                            onChange={(e) =>
                              handleStipulationChange(doc._id, e.target.value)
                            }
